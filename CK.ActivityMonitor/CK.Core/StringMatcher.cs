...
namespace CK.Core
{
    ...
    public sealed class StringMatcher
    {
        ...
        public bool Forward( int charCount )
        {
            ...
            if( newLen < 0 ) throw new InvalidOperationException( R.StringMatcherForwardPastEnd );
        ...
        {
            return TryMatchChar( c ) ? Forward(0) : SetError( c );
        }
        ...
        public bool TryMatchString( string s, StringComparison comparisonType = StringComparison.OrdinalIgnoreCase )
        {
            ...
            return !IsEnd
                    && len <= _length
                    && String.Compare( s, _startIndex, s, 0, len, comparisonType ) == 0
                ? UncheckedMove( len )
                : false;
        }
