#region LGPL License
/*----------------------------------------------------------------------------
* This file (CK.Tests\Kernel\Modes\ContextModes.cs) is part of CiviKey. 
*  
...
namespace Keyboard
{
    ...
    /// <summary>
    /// This class test operations on IKeyboardMode (ObtainMode, Intersect, etc.).
    /// </summary>
    [TestFixture]
    public class ContextModes
    {
        ...
        {
            IKeyboardMode m = Context.EmptyMode;
        ...
        {
            IKeyboardMode m = Context.ObtainMode( "Alpha" );
        ...
        {
            IKeyboardMode m = Context.ObtainMode( "Beta+Alpha" );
        ...
        {
            IKeyboardMode m1 = Context.ObtainMode( "Beta+Alpha+Fridge+Combo" );
            IKeyboardMode m2 = Context.ObtainMode( "Xtra+Combo+Another+Fridge+Alt" );

            Assert.That( m1.Intersect( m2 ).ToString() == "Combo+Fridge", "Works as expected :-)" );
            Assert.That( m2.Intersect( m1 ) == m1.Intersect( m2 ), "Same in both calls." );
        ...
        {
            IKeyboardMode m = Context.ObtainMode( "Beta+Alpha+Fridge+Combo" );
        ...
        {
            {
                IKeyboardMode m = Context.ObtainMode( "" );
                IReadOnlyList<IKeyboardMode> f = m.Fallbacks;
            ...
            {
                IKeyboardMode m = Context.ObtainMode( "Alpha+Beta+Combo" );
                IReadOnlyList<IKeyboardMode> f = m.Fallbacks;
            ...
            {
                IKeyboardMode m = Context.ObtainMode( "Alpha+Beta+Combo+Fridge" );
                IReadOnlyList<IKeyboardMode> f = m.Fallbacks;
