...
namespace CK.Core
{
    ...
    public readonly struct SHA512Value : IEquatable<SHA512Value>, IComparable<SHA512Value>
    {
        ...
        /// <summary>
        /// Tries to match a 128 length hexadecimal string to a SHA512 value.
        /// </summary>
        /// <param name="head">The text to parse.</param>
        /// <param name="value">The value on success, <see cref="Zero"/> on error.</param>
        /// <returns>True on success, false on error.</returns>
        public static bool TryMatch( ref ReadOnlySpan<char> head, out SHA512Value value )
        {
            value = Zero;
            if( head.Length < 128 ) return false;
            try
            {
                var h = head.Slice( 0, 128 );
                ...
                value = bytes.SequenceEqual( Zero._bytes ) ? Zero : new SHA512Value( bytes, new string( h ) );
                head = head.Slice( 128 );
                return true;
            }
        ...
        /// <param name="value">The value on success, <see cref="Zero"/> on error.</param>
        /// <returns>True on success, false on error.</returns>
        public static bool TryParse( ReadOnlySpan<char> text, out SHA512Value value ) => TryMatch( ref text, out value ) && text.IsEmpty;

        /// <summary>
        /// Parses a 128 length hexadecimal string to a SHA512 value or throws an <see cref="ArgumentException"/>.
        /// </summary>
        ...
        /// <summary>
        /// Initializes a new <see cref="SHA512Value"/> from the current value of an <see cref="IncrementalHash"/>
        ...
        {
            Throw.DebugAssert( b != null && b.Length == 64 );
        ...
        {
            Throw.DebugAssert( b.Length == 64 && !b.SequenceEqual( Zero._bytes ) && s != null && s.Length == 128 );
            _bytes = b;
            _string = s;
        }
