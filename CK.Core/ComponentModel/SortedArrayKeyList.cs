#region LGPL License
/*----------------------------------------------------------------------------
* This file (CK.Core\ComponentModel\SortedArrayKeyList.cs) is part of CiviKey. 
*  
...
namespace CK.Core
{
    ...
    public class SortedArrayKeyList<T, TKey> : SortedArrayList<T>, IReadOnlyMultiKeyedCollection<T, TKey>
    {
        ...
        /// <summary>
        /// Initializes a new <see cref="SortedArrayKeyList{T,TKey}"/> where a <see cref="Comparison{T}"/> function
        ...
        /// <summary>
        /// Gets the number of items with a given key. It can be greater than 1 only if <see cref="SortedArrayList{T}.AllowDuplicates">AllowDuplicates</see> is true.
        /// </summary>
        /// <param name="key">The key to find.</param>
        ...
        public IReadOnlyCollection<T> GetAllByKey( TKey key )
        {
            ...
            if( !AllowDuplicates ) return new ReadOnlyListMono<T>( Store[idx] );
            int min = idx - 1;
            while( min >= 0 && ComparisonKey( Store[min], key ) == 0 ) --min;
            int max = idx + 1;
            while( max < Store.Length && ComparisonKey( Store[max], key ) == 0 ) ++max;           
        ...
        /// the <see cref="SortedArrayList{T}.Contains(T)"/> is used but if <paramref name="item"/> is of type <typeparamref name="TKey"/>,
        ...
        /// <summary>
        /// Removes one item given a key: only one item is removed when <see cref="SortedArrayList{T}.AllowDuplicates"/> is 
